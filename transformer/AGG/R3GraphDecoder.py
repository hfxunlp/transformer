...
class DecoderLayer(nn.Module):
	...
	def __init__(self, isize, fhsize=None, dropout=0.0, attn_drop=0.0, num_head=8, ahsize=None, num_sub3=1, num_sub=1, num_unit=1):
...
class Decoder(DecoderBase):
	...
	def __init__(self, isize, nwd, num_layer, fhsize=None, dropout=0.0, attn_drop=0.0, emb_w=None, num_head=8, xseql=512, ahsize=None, norm_output=False, bindemb=False, forbidden_index=None, num_sub3=1, num_sub=1, num_unit=1):
	...
	def beam_decode(self, inpute, src_pad_mask=None, beam_size=8, max_len=512, length_penalty=0.0, return_all=False, clip_beam=False):
		...
		for key, value in states.items():
			if value.dim() == 6:
				states[key] = value.repeat(1, beam_size, 1, 1, 1, 1).view(real_bsize, 1, -1, value.size(-3), value.size(-2), isize)
			else:
