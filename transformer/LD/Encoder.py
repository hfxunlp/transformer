...
class EncoderLayer(EncoderLayerBase):
	...
	def forward(self, inputs, sumr, mask=None, rmask=None):
...
class Encoder(EncoderBase):
	...
	def __init__(self, isize, nwd, num_layer, fhsize=None, dropout=0.0, attn_drop=0.0, num_head=8, xseql=cache_len_default, ahsize=None, norm_output=True, num_layer_dec=6, max_chunk_tokens=8, min_chunks=4):
	...
	def forward(self, inputs, mask=None):
		...
		if self.pemb is not None:
			...
			#_nele = (_ntok - _nmask).view(bsize, _nchk, 1).to(out)
